...
for(const [token,model] of [['HERO_PRIMARY','iPhone 14 Pro Max'],['HERO_SECOND','iPhone 17 Pro Max'],['HERO_THIRD','iPhone 16 Pro Max']]) {
...
html=html.replace('{{HERO_PRICE}}',priceText(storeItems.find(p=>p.slug==='iphone-14-pro-max')));
...
for(const c of shopCategories){
 ...
 const page=html.replace(/<main id="main">[\s\S]*?<\/main>/,`<main id="main" class="store-section category-detail"><a class="text-link" href="/?category=${c.id}#products">← Back to devices</a><div class="category-detail-grid">${media(c.image,c.name,true)}<div><p class="eyebrow">MIKEE GADGET PLUG</p><h1>${escape(c.name)}</h1><p>Find your next upgrade. Shop in Computer Village or order for delivery.</p><a class="btn btn-primary" href="${contact}">See options on WhatsApp ↗</a><dl><div><dt>Price &amp; stock</dt><dd>Models and prices vary. Check availability before payment.</dd></div><div><dt>Collection</dt><dd>Pickup in Ikeja or delivery across Nigeria</dd></div><div><dt>Payment</dt><dd>Pay outright. EasyBuy is subject to device eligibility and approval.</dd></div></dl><a class="call-link" href="tel:${site.telephoneHref}">Call ${site.telephoneDisplay}</a></div></div></main>`).replace(/<title>.*?<\/title>/,`<title>${escape(title)}</title>`)
