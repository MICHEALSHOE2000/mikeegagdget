...
for(const c of shopCategories){
 ...
 const description=`Shop ${c.name} at Mikee Gadget Plug in Computer Village, Ikeja. Ask for current models and prices. Store pickup or delivery across Nigeria.`;
 ...
 const page=html.replace(/<main id="main">[\s\S]*?<\/main>/,`<main id="main" class="store-section category-detail"><a class="text-link" href="/?category=${c.id}#products">← Back to devices</a><div class="category-detail-grid">${media(c.image,c.name,true)}<div><p class="eyebrow">MIKEE GADGET PLUG</p><h1>${escape(c.name)}</h1><p>Choose your model, condition and budget. We’ll send you the current options.</p><dl><div><dt>Price &amp; stock</dt><dd>Confirm current options</dd></div><div><dt>Collection</dt><dd>Pickup in Ikeja or delivery across Nigeria</dd></div><div><dt>Payment</dt><dd>Pay outright. Ask about EasyBuy eligibility.</dd></div></dl><a class="btn btn-primary" href="${contact}">See options on WhatsApp ↗</a><a class="call-link" href="tel:${site.telephoneHref}">Call ${site.telephoneDisplay}</a></div></div></main>`).replace(/<title>.*?<\/title>/,`<title>${escape(title)}</title>`)
